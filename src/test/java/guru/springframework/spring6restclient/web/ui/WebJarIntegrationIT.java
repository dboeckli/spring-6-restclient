...
class WebJarIntegrationIT {
    ...
    void shouldResolveBootstrapCssWithoutVersion() throws Exception {
        ...
        // Dies stellt sicher, dass 'webjars-locator-lite' korrekt konfiguriert ist und
        // funktioniert.
        mockMvc.perform(get("/webjars/bootstrap/css/bootstrap.min.css"))
            .andExpect(status().isOk()) // Erwartet HTTP 200
            .andExpect(content().contentTypeCompatibleWith(MediaType.valueOf("text/css"))); // Prüft
                                                                                            // ob
                                                                                            // auch
                                                                                            // wirklich
                                                                                            // CSS
    ...
    void shouldResolveBootstrapJsWithoutVersion() throws Exception {
        ...
        mockMvc.perform(get("/webjars/bootstrap/js/bootstrap.min.js")).andExpect(status().isOk());
    }
